...
export const Preloader: React.FC<PreloaderProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    // Phase 1: Show Text (Almost immediately)
    setTimeout(() => setShowContent(true), 100);

    // Phase 2: Start Exit Animation (Faster reading time)
    setTimeout(() => {
      setExiting(true);
    }, 1000);

    // Phase 3: Unmount (Total duration ~1.6s)
    setTimeout(() => {
      onComplete();
    }, 1600); // 1000ms + 600ms exit transition
  }, [onComplete]);

  return (
    <div 
      className={`fixed inset-0 z-[100] flex items-center justify-center bg-navy-950 transition-transform duration-[600ms] cubic-bezier(0.83, 0, 0.17, 1) ${exiting ? '-translate-y-full' : 'translate-y-0'}`}
    >
      {/* Background Ambience */}
      ...
      {/* Main Content */}
      <div className={`relative flex flex-col items-center transition-all duration-700 transform ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        {/* Logo Icon */}
        ...
        <div className="h-px w-0 bg-gradient-to-r from-transparent via-gold-400 to-transparent mt-4 animate-[shimmer_2s_infinite]" style={{ width: showContent ? '100%' : '0%', transition: 'width 0.8s ease-out' }}></div>
